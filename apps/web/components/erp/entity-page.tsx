    ...
    <div>
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-[#788983]">{schema.description}</p>
        </div>
        <div className="flex gap-2">
          <button
            type="button"
            onClick={exportCsv}
            className="rounded-xl border border-[#dfe7e3] bg-white px-4 py-2.5 text-xs font-semibold text-[#53655e] hover:bg-[#f8faf9]"
          >
            تصدير Excel
          </button>
          <button
            type="button"
            onClick={openCreate}
            className="flex items-center gap-2 rounded-xl bg-[#123c35] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#1d594d]"
          >
            <Plus size={15} />
            إضافة جديد
          </button>
      ...
      <section className="rounded-2xl border border-[#e1e9e5] bg-white p-5 shadow-[0_4px_22px_rgba(31,65,53,0.04)]">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="font-bold">السجلات</h3>
            <p className="mt-1 text-xs text-[#899892]">{filtered.length} سجل — الحقول مطابقة لنموذج البيانات</p>
          </div>
          <div className="relative">
            <Search size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9aa9a3]" />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="بحث..."
              className="h-9 rounded-lg border border-[#dfe7e3] pr-9 pl-3 text-xs outline-none focus:border-[#1d7f72]"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-right">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 2} className="py-10 text-center text-sm text-[#899892]">
                ...
                filtered.map((row) => (
                  <tr key={row.id} className="border-b border-[#f0f4f2] last:border-0">
                    <td className="py-3 text-[11px] font-semibold text-[#50635b]">{row.id}</td>
                    {columns.map((col) => (
                      <td key={col.key} className="py-3 text-xs text-[#53655e]">
                    ...
                    <td className="py-3">
                      <div className="flex gap-1">
                        <button
                          ...
                          className="rounded-lg border border-[#dfe7e3] p-1.5 text-[#53655e] hover:bg-[#f8faf9]"
                        >
                          <Pencil size={14} />
                        </button>
                        <button
                          ...
                          className="rounded-lg border border-[#f0d0c8] p-1.5 text-[#ad5e46] hover:bg-[#fff5f2]"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
          ...
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-[#899892]">{schema.title} — حقول النموذج من الـ Schema</p>
              </div>
              <button type="button" onClick={() => setDialogOpen(false)} className="text-[#899892] hover:text-[#123c35]">
                إغلاق
              </button>
            </div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button
                ...
                className="rounded-xl border border-[#dfe7e3] px-4 py-2.5 text-xs font-semibold text-[#53655e]"
              >
                إلغاء
              </button>
              <button
                type="button"
                onClick={save}
                className="rounded-xl bg-[#123c35] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#1d594d]"
              >
                حفظ
              </button>
            </div>
