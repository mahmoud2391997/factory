...
function ModuleView({ module, search, onSearchChange, onCreate }: { module: { title: string; description: string; stats: string[]; rows: string[][] }; search: string; onSearchChange: (value: string) => void; onCreate: () => void }) {
  const filteredRows = module.rows.filter((row) => row.join(' ').includes(search.trim()))
  ...
  return <div>
    ...
    <section className="rounded-2xl border border-[#e1e9e5] bg-white p-5 shadow-[0_4px_22px_rgba(31,65,53,0.04)]"><div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-bold">السجل التشغيلي</h3><p className="mt-1 text-xs text-[#899892]">بيانات قابلة للبحث والتصفية والتدقيق</p></div><div className="flex gap-2"><input value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="بحث..." aria-label="بحث في السجل" className="h-9 rounded-lg border border-[#dfe7e3] px-3 text-xs outline-none focus:border-[#1d7f72]" /><select className="rounded-lg border border-[#dfe7e3] bg-white px-3 text-xs text-[#53655e]"><option>كل الحالات</option><option>نشط</option><option>مكتمل</option><option>متأخر</option></select></div></div><div className="overflow-x-auto"><table className="w-full min-w-[620px] text-right"><thead><tr className="border-b border-[#edf2ef] text-[11px] text-[#97a49f]"><th className="pb-3 font-medium">المرجع</th><th className="pb-3 font-medium">التفاصيل</th><th className="pb-3 font-medium">القيمة / الحالة</th><th className="pb-3 font-medium">الحالة</th></tr></thead><tbody>{filteredRows.map((row) => <tr key={row[0]} className="border-b border-[#f0f4f2] last:border-0"><td className="py-4 text-xs font-semibold text-[#50635b]">{row[0]}</td><td className="py-4 text-xs text-[#53655e]">{row[1]}</td><td className="py-4 text-xs text-[#53655e]">{row[2]}</td><td className="py-4"><span className="rounded-full bg-[#e6f4ef] px-2.5 py-1 text-[10px] font-semibold text-[#19725f]">{row[3]}</span></td></tr>)}</tbody></table></div></section>
  </div>
}
